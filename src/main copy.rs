...
fn calculate_balance_changes(
    ...
    // First let's verify that the sum of inputs and outputs match for every transaction
    ...
    // If sum of inputs and outputs does not match, the transaction must be rejected
    if input_sum != output_sum {
        return Err("Sum of inputs and outputs does not match.".to_string());
    }

    // TODO: Apply burn_rate and commission_rate as described by their definition.
    // TODO: Check if the sender has enough balances to cover the input amount on top of burn_rate and commission_rate.
    // TODO: Calculate and apply the burn and commission for the issuer.

    // If everything is correct, return the original_balances for now
    Ok(original_balances)
}

// TODO: Write different unit tests to cover all the edge cases
#[cfg(test)]
mod tests {
    use super::*;
